...
import { Send, Phone, User, CheckCheck, Bot, Sparkles } from 'lucide-react';
...
export const WhatsAppChat: React.FC<WhatsAppChatProps> = ({ onBookProvider }) => {
  const [messages, setMessages] = useState<ChatMessage[]>([
    {
      id: 'welcome',
      text: "Hello! 👋 Welcome to FundiConnect. I'm your AI assistant ready to help you find the perfect service provider. What service are you looking for today?",
      isBot: true,
      timestamp: new Date(),
      type: 'text'
    }
  ...
  const handleSendMessage = async () => {
    ...
    setTimeout(async () => {
      const botResponses = await botService.processMessage(inputText);
  ...
  const QuotationCard: React.FC<{ quotation: ServiceQuotation }> = ({ quotation }) => (
    <div className="bg-gradient-to-r from-green-50 to-emerald-50 border-l-4 border-green-400 p-4 rounded-xl mb-3 shadow-lg">
      ...
      <div className="flex justify-between items-center text-sm text-green-600">
        ...
        <button
          ...
          className="bg-gradient-to-r from-green-500 to-emerald-600 text-white px-4 py-2 rounded-full text-sm font-semibold hover:shadow-lg transition-all duration-300 transform hover:scale-105"
        >
          Book Now
        </button>
      </div>
  ...
  const suggestedMessages = [
    ...
    "Need a mason for stone wall construction"
  ];
    ...
    <div className="h-[700px] bg-white/90 backdrop-blur-lg rounded-3xl overflow-hidden flex flex-col shadow-2xl border border-white/20">
      ...
      <div className="bg-gradient-to-r from-green-600 via-emerald-600 to-green-700 text-white p-6 flex items-center space-x-4 shadow-lg">
        ...
        <div className="flex-1">
          <h3 className="font-bold text-lg">FundiConnect AI</h3>
          <p className="text-green-100 text-sm flex items-center space-x-2">
            <span className="w-2 h-2 bg-green-300 rounded-full animate-pulse"></span>
            <span>Online • AI-Powered Assistant</span>
          </p>
        </div>
      ...
      {messages.length <= 1 && (
        <div className="px-6 pb-4">
          <p className="text-sm text-gray-500 mb-3 font-medium">Quick suggestions:</p>
